...
$(document).ready(function(){
	...
	$('#telYear1').change(function() {
		var reservationDate = $.trim($('#telYear1').val().replace(/-/g,''));
		$.ajax({
			type : "POST",
	...
	$('#telYear2').change(function() {
		var reservationDate = $.trim($('#telYear2').val().replace(/-/g,''));
		$.ajax({
			type : "POST",
			url : "/customerCenter/checkReservationDate.eds",
			data : {
...
// 두번째 예약시간 날짜 세팅
function setTelYear2() {
	if($('#telYear1').val() != "") {
		var firstDateStr = $('#telYear1').val();
		var secondDate = new Date(firstDateStr)
		
		// 오늘 날짜 하루 더함.
		var tomorrow = new Date(Date.parse(secondDate) + 1 * 1000 * 60 * 60 * 24); // 오늘 날짜의 하루 더함.
		year = (tomorrow.getFullYear()).toString();
		month = (tomorrow.getMonth() + 1).toString();
		if (month.length == 1) {
			month = '0' + month;
...
function checkServiceTime() {
	$.ajax({
		...
		success : function(result) {
			if (result.success) {
				// 채팅상담 ON
				if (result.chkChat == 'Y' && insuranceType != '49') {
					$('em[name=chatStatus]').removeClass('off');
					$('em[name=chatStatus]').addClass('on');
					$('em[name=chatStatus]').text('ON');
				}

				// 이메일상담, 전화상담 - 이름
				if (result.chkEmail == 'Y' && insuranceType != '49') {
					$('em[name=emailStatus]').removeClass('off');
					$('em[name=emailStatus]').addClass('on');
					$('em[name=emailStatus]').text('ON');
				...
				// 전화상담
				if (result.chkTel == 'Y') {
					$('em[name=telStatus]').removeClass('off');
					$('em[name=telStatus]').addClass('on');
					$('em[name=telStatus]').text('ON');
				}

				// 전화상담예약 ON
				if (result.chkTelReserve == 'Y' && insuranceType != '49') {
					$('em[name=telReserveStatus]').removeClass('off');
					$('em[name=telReserveStatus]').addClass('on');
					$('em[name=telReserveStatus]').text('ON');
				...
				// 원격지원 ON
				if (result.chkRemote == 'Y' && insuranceType != '49') {
					$('#remoteStatus').removeClass('off');
					$('#remoteStatus').addClass('on');
					$('#remoteStatus').text('ON');
					$('em[name=remoteStatus]').removeClass('off');
					$('em[name=remoteStatus]').addClass('on');
					$('em[name=remoteStatus]').text('ON');
				}
				...
				if(insuranceType != '49'){
					...
					checkServiceChat, checkServiceEmail, checkServiceTelReserve, checkServiceRemote = 'N';
					checkServiceTel = result.chkTel;
				}
			} else {
				// 채팅상담 OFF
				$('em[name=chatStatus]').removeClass('on');
				$('em[name=chatStatus]').addClass('off');
				$('em[name=chatStatus]').text('OFF');

				// 전화상담 OFF
				$('em[name=telStatus]').removeClass('on');
				$('em[name=telStatus]').addClass('off');
				$('em[name=telStatus]').text('OFF');

				// 원격지원 OFF
				$('#remoteStatus').removeClass('on');
				$('#remoteStatus').addClass('off');
				$('#remoteStatus').text('OFF');
				$('em[name=remoteStatus]').removeClass('on');
				$('em[name=remoteStatus]').addClass('off');
				$('em[name=remoteStatus]').text('OFF');
...
// 상담팝업 실행
function openPopCounsel(counselType) {
	checkServiceTime();
	...
	if (counselType == 'chat') {
		if (checkServiceChat == 'Y'){
			...
			if(checkServiceTimeMessage) alert(checkServiceTimeMessage);
			return;
		}
	}
	...
	// 무료전화상담
	if (counselType == 'tel') {
		if (checkServiceTel == 'Y'){
			if(insuranceType == '49'){
				location.href="tel:15883114";
			}else{
				location.href="tel:0807893300";
			}
		} else {
			if(checkServiceTimeMessage) alert(checkServiceTimeMessage);
			return;
...
function sendTelCounsel() {
	...
	if($('#telYear2').is(':visible')){
		telYear2 = $.trim($('#telYear2').val().replace(/-/g,'')); // 두번째 예약 날짜
		telStartTime2 = $('#telStartTime2 option:selected').text();
		telEndTime2 = $('#telEndTime2 option:selected').text();
	}
	...
	$.ajax({
		...
		success : function(result) {
			if (!result.success) {
				...
				if($('#addTel').is(':visible')){
					if (reserveTime2.toString().length == 10) {
						if (reserveTime2 <= nowTime) {
							alert("예약시간은 현재시간 이후로 선택해 주세요.");
							$('#telYear2').focus();
							return;
						}
				...
				//상담신청 가능시간 당일부터 2주 이내로
				var telCounselAvailableDate = new Date(d);
				telCounselAvailableDate.setDate(telCounselAvailableDate.getDate() + 15);
				var telCounselAvailableTime = dateToString(telCounselAvailableDate);
				
				if(reserveTime.toString().substring(0, 8) >= telCounselAvailableTime.substring(0, 8)){
					alert('상담신청은 2주 이내의 날짜만 선택가능합니다.\n(' + telCounselAvailableTime.substring(4, 6) + '월 ' 
							+ telCounselAvailableTime.substring(6, 8) + '일 전까지 선택가능)');
					$('#telYear1').focus();
					return;
				}
				if($('#addTel').is(':visible')){
					if(reserveTime2.toString().substring(0, 8) >= telCounselAvailableTime.substring(0, 8)){
						alert('상담신청은 2주 이내의 날짜만 선택가능합니다.\n(' + telCounselAvailableTime.substring(4, 6) + '월 ' 
								+ telCounselAvailableTime.substring(6, 8) + '일 전까지 선택가능)');
						$('#telYear2').focus();
						return;
...
function chattingPop(){
	// ga('send','event','Internet','Click',"Step1_Chat",1); // 기본형
...
/* Date 객체를 String(yyyymmddhh)으로 반환 */
